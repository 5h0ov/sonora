...
library-no-artists = お気に入りのアーティストはまだいません
...
library-no-catalog-songs = 曲が見つかりません
library-no-catalog-albums = アルバムが見つかりません
library-no-catalog-artists = アーティストが見つかりません
...
confirm-remove-artists = { $count ->
   *[other] { $count }人のアーティストをお気に入りから削除しますか？
}
...
login-cookie-step-2 = F12 を押して Network タブを開き、ページを再読み込みします。
...
login-server-title = Subsonic サーバーに接続
login-server-detail = Subsonic または OpenSubsonic サーバー（Navidrome、Airsonic、Gonic など）のアドレスを入力し、サーバーのユーザー名とパスワードでサインインしてください。セッションはこのデバイスに保存されます。
login-server-hint = https://music.example.com
login-username-hint = ユーザー名
login-password-hint = パスワード
login-server-submit = 接続
...
settings-visualizer-detail = フルスクリーンのアートワークの背後にスペクトラムバーを表示する
settings-fullscreen-controls-autohide = フルスクリーンのコントロールを隠す
settings-fullscreen-controls-autohide-detail = フルスクリーン中に操作がないとき再生コントロールをフェードアウトさせる
...
settings-blur = UI をぼかす
settings-blur-detail = メニューや浮いた操作部をすりガラスにする
settings-blur-window = ウィンドウをぼかす
settings-blur-window-detail = ぼかしたデスクトップの上にウィンドウを描画する。不透明度を 100% 未満にする必要がある
...
settings-server-side-decorations = サーバーサイドデコレーション
settings-server-side-decorations-detail = タイトルバー、枠線、影をコンポジタに描画させる
...
settings-traffic-light-controls = 信号機風コントロール
settings-traffic-light-controls-detail = 最小化・最大化・閉じるを色付きドットで表示する
settings-window-rounding = ウィンドウの角の丸み
settings-window-rounding-detail = ウィンドウ自体の角の丸め具合
settings-controls-side = コントロールの位置
settings-controls-side-detail = タイトルバーのどちら側にコントロールを置くか
settings-close-to-tray = 閉じても再生を続ける
settings-close-to-tray-detail = ウィンドウを閉じたあとも Sonora をトレイに残して再生を続ける
settings-discord = Discord に表示
settings-discord-detail = 再生中の曲を Discord のプロフィールに表示する
settings-discord-name = ステータス名
settings-discord-name-detail = フレンドに見える「再生中」の後に続くステータスの名前
settings-discord-name-sonora = Sonora
settings-discord-name-provider = サービス
settings-discord-name-music = 音楽
settings-discord-name-title = タイトル
settings-discord-name-artist = アーティスト
settings-discord-name-artist-title = アーティスト - タイトル
settings-discord-show-paused = 一時停止中も表示
settings-discord-show-paused-detail = 曲が一時停止していても Discord のプロフィールにステータスを残す
settings-discord-badge = サービスのバッジを表示
settings-discord-badge-detail = 曲の提供元サービスの小さなアイコンをステータスに付ける
settings-discord-anonymous = 詳細を隠す
settings-discord-anonymous-detail = タイトル、アーティスト、アートワークを出さず、音楽を再生中であることだけを伝える
settings-discord-buttons = ボタン
settings-discord-buttons-detail = ステータスの下にフレンドが開けるリンクを置く。ひとつは提供元サービスの曲へ、もうひとつは Sonora へ
settings-discord-buttons-pick = ボタンを選ぶ
# the Discord status when the track is left out of it
discord-listening = 音楽を再生中
get-sonora = Sonora を入手
settings-fullscreen-controls-autohide-automatic = 自動
settings-fullscreen-controls-autohide-always-hidden = 常に隠す
settings-fullscreen-controls-autohide-always-shown = 常に表示
...
settings-sleep = スリープタイマー
settings-sleep-detail = 設定した時間が過ぎると音楽が自動で止まり、眠りにつくまで再生できる
settings-sleep-configure = 設定…
settings-sleep-off = オフ
settings-sleep-end-of-track = 曲の終わり
settings-sleep-minutes = { $count }分
settings-equalizer = イコライザー
settings-equalizer-detail = 1 オクターブごとに 10 バンドで音を整える
settings-equalizer-preset = プリセット
settings-equalizer-preset-detail = 下のバンドにそのまま使える設定
settings-equalizer-custom = カスタム
settings-equalizer-flat = フラット
settings-equalizer-bass-boost = 低音ブースト
settings-equalizer-bass-reducer = 低音カット
settings-equalizer-treble-boost = 高音ブースト
settings-equalizer-vocal = ボーカル
settings-equalizer-rock = ロック
settings-equalizer-pop = ポップ
settings-equalizer-jazz = ジャズ
settings-equalizer-classical = クラシック
settings-equalizer-electronic = エレクトロニック
settings-equalizer-acoustic = アコースティック
settings-equalizer-loudness = ラウドネス
settings-equalizer-decibels = { $db } dB
settings-equalizer-hertz = { $hz } Hz
settings-equalizer-kilohertz = { $khz } kHz
settings-panel-lyrics-size = 歌詞のサイズ（パネル）
settings-panel-lyrics-size-detail = サイドパネルの歌詞の文字サイズ。基準のフォントサイズに対する倍率
settings-fullscreen-lyrics-size = 歌詞のサイズ（フルスクリーン）
settings-fullscreen-lyrics-size-detail = フルスクリーンプレイヤーの歌詞の文字サイズ。基準のフォントサイズに対する倍率
settings-lyrics-size-value = { $size }%
settings-lyrics-for-local-files = ローカルファイルの歌詞
settings-lyrics-for-local-files-detail = ローカルファイルのメタデータを使ってインターネットから歌詞を取得する
settings-karaoke-lyrics = カラオケ歌詞
settings-karaoke-lyrics-detail = タイミング情報があるとき歌詞を単語ごとにハイライトする
settings-blur-lyrics = 再生中以外の歌詞をぼかす
settings-blur-lyrics-detail = 歌詞パネルで前後の行をぼかす
...
settings-group-window-style = ウィンドウのスタイル
settings-group-lyrics = 歌詞
settings-group-equalizer = イコライザー
...
settings-add-folder = フォルダを追加
settings-remove-folder = フォルダを削除
...
nav-pins-kind = 種類別
nav-show-full-library = ライブラリ全体を表示
nav-return-top = 先頭に戻る
